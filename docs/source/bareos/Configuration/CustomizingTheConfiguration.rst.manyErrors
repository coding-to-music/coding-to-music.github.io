...
.. image:: /bareos/include/images/bareos-read-configuration.*
...
-  The correct way of migrating to the new configuration scheme would be to split the configuration file into resources, store them in the resource directories and then remove the original configuration file.
   ...
   -  For migrating the |dir| configuration, the script `bareos-migrate-config.sh <https://github.com/bareos/blob/master/contrib/misc/bareos-migrate-config/bareos-migrate-config.sh>`_ exists. Being called, it connects via :command:`bconsole` to a running |dir| and creates subdirectories with the resource configuration files.
      ...
      .. code-block:: shell-session
         ...
         wget https://raw.githubusercontent.com/bareos/master/contrib/misc/bareos-migrate-config/bareos-migrate-config.sh
...
.. figure:: /bareos/include/images/Conf-Diagram.*
